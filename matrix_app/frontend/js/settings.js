document.addEventListener('DOMContentLoaded', () => {
    ...
    // ════════════════════════════════════════════════════════════════
    // VIZ WITH CHAT TOGGLE
    // ════════════════════════════════════════════════════════════════
    const vizWithChatToggle = document.getElementById('vizWithChatToggle');
    vizWithChatToggle?.addEventListener('change', () => {
        localStorage.setItem('vizWithChatEnabled', vizWithChatToggle.checked ? 'true' : 'false');
    });
    ...
    function updateSettingsUI() {
        ...
        // Update viz with chat toggle
        if (typeof vizWithChatToggle !== 'undefined' && vizWithChatToggle) {
            vizWithChatToggle.checked = localStorage.getItem('vizWithChatEnabled') === 'true';
        }
        
        // Or if it was not in scope because we re-fetched it:
        const localVizToggle = document.getElementById('vizWithChatToggle');
        if (localVizToggle) {
            localVizToggle.checked = localStorage.getItem('vizWithChatEnabled') === 'true';
        }
    ...
    function updateDynamicTranslations() {
        ...
        if (locationText && locationText.textContent === 'Detecting...') {
            if (window.i18n) {
                locationText.textContent = window.i18n.t('detectingLocation');
            }
        }
    ...
    // ════════════════════════════════════════════════════════════════
    const PROFILE_SETTINGS_KEYS = ['theme', 'relatedLinksEnabled', 'vizWithChatEnabled', 'likeDislikeEnabled'];
...
window.settingsManager = {
    ...
    isVizWithChatEnabled: () => localStorage.getItem('vizWithChatEnabled') === 'true',
