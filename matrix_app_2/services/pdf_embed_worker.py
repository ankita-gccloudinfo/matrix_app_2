...
# ceiling, so reranking computation is isolated here and exposed over a tiny
...
#
# Embedding used to be served from here too (local SentenceTransformer), but
# that now goes straight to a vLLM-hosted jina-embeddings-v3 over HTTP (see
# ../embedder.py) — this worker only handles the CrossEncoder reranker, which
# has no vLLM equivalent.
import os
import threading
...
def get_device():
    ...
    # Need a bit of headroom for the reranker
...
print(f" Loading reranker model {RERANK_MODEL_NAME} on {_DEVICE} "
      f"({'GPU: ' + torch.cuda.get_device_name(_DEVICE) if _DEVICE.startswith('cuda') else f'{torch.get_num_threads()} CPU threads'})...")
_reranker = CrossEncoder(RERANK_MODEL_NAME, device=_DEVICE, max_length=512)
print(f" Reranker model loaded on {_DEVICE}.")

app = FastAPI(title="PDF Rerank Worker")
...
@app.get("/health")
def health():
    return {"status": "ok", "model": RERANK_MODEL_NAME, "device": _DEVICE}
...
if __name__ == "__main__":
    ...
    # (spawned as a local subprocess, see load_rerank_worker()), never from a
